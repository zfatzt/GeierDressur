...
public class PluginManager extends JPanel implements ICCPlugin {
	...
	private void init() {
		...
		// now we have a sorted list of class names and can go for creating
		// panels
		for (int i = 0; i < sortedList.size(); i++) {
			// for each plugin class a new panel (with a button to (de)activate
			...
			try {
				...
				contentPane.add(panel,
						new GridBagConstraints(0, contentPane.getComponentCount(), 1, 1, 1, 0,
		...
		contentPane.add(new JLabel(""), new GridBagConstraints(0, contentPane.getComponentCount(), 1, 1, 1, 1,
				GridBagConstraints.FIRST_LINE_START, GridBagConstraints.BOTH, new Insets(0, 0, 0, 0), 0, 0));

		add(new JScrollPane(contentPane), new GridBagConstraints(0, 0, 1, 1, 1, 1, GridBagConstraints.FIRST_LINE_START,
	...
	private JPanel createPluginPanel(final ICCPlugin plugin) {
		...
		panel.add(button, new GridBagConstraints(0, 0, 1, 1, 0, 0, GridBagConstraints.FIRST_LINE_START,
				GridBagConstraints.NONE, new Insets(0, 0, 0, 0), 0, 0));
		panel.add(new JLabel("<html><i>" + plugin.getDescription() + "</i></html>"),
				new GridBagConstraints(1, 0, 1, 1, 1, 0, GridBagConstraints.FIRST_LINE_START,
	...
	private JPanel createErrorPanel(String pluginClass, Exception exc) {
		...
		panel.add(
				new JLabel("<html><font color=#ff0000>Failed to load plugin: " + exc.getMessage() + " ("
						+ exc.getClass().getSimpleName() + ")</font></html>"),
	...
	private void initPluginInternalFrame(final ICCPlugin plugin, JInternalFrame frame) {
		...
		frame.setSize(pluginProperties.isPluginAutoStart(plugin.getTitle())
				? pluginProperties.getPluginScreenSize(plugin.getTitle()) : plugin.getScreenSize());
		frame.setLocation(pluginProperties.isPluginAutoStart(plugin.getTitle())
				? pluginProperties.getPluginScreenLocation(plugin.getTitle()) : plugin.getScreenLocation());
